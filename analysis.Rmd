...
We first tried statistical comparison using McNemar's Chi-squared Test for Count Data (test on paired proportions). See https://www.medcalc.org/manual/mcnemartest2.php for background on McNemar's Test and https://www.rdocumentation.org/packages/stats/versions/3.6.0/topics/mcnemar.test for R documentation. We have included the code below, but we are not evaluating this because it does not measure quite what we are looking for. Read on!
...
paste("McNemar test: p =", round(mcnemar_result$p.value, 4)); paste("d =", round(mcnemar_effect_size$d, 2))
...
paste("McNemar test: p =", round(mcnemar_result2$p.value, 4)); paste("d =", round(mcnemar_effect_size2$d, 2))
...
paste("McNemar test: p =", round(mcnemar_result3$p.value, 4)); paste("d =", round(mcnemar_effect_size3$d, 2))




edchat_monthly_volume <- matrix(c(n_orig_tweets, n_retweets,
                                    n_orig_tweets_unspammed, n_retweets_unspammed),
                                  nrow = 2,
                                  dimnames = list("Tweet Type" = c("Original", "Retweet"),
                                                  "Spam Removal" = c("Before", "After")
                                                  )
                                  )
edchat_monthly_volume
exact2x2::mcnemar.exact(edchat_monthly_volume, conf.level=.95)
mcnemar.test(edchat_monthly_volume, correct = TRUE)

activity_orig_before <- n_orig_tweets / n_months_rtweet / n_orig_tweeters
activity_orig_after <- n_orig_tweets_unspammed / n_months_unspammed / n_orig_tweeters_unspammed
activity_rt_before <- n_retweets / n_months_rtweet / n_retweeters
activity_rt_after <- n_retweets_unspammed / n_months_unspammed / n_retweeters_unspammed

edchat_monthly_activity <- matrix(c(activity_orig_before, activity_rt_before,
                                    activity_orig_after, activity_rt_after),
                                  nrow = 2,
                                  dimnames = list("Tweet Type" = c("Original", "Retweet"),
                                                  "Spam Removal" = c("Before", "After")
                                                  )
                                  )
edchat_monthly_activity
exact2x2::mcnemar.exact(edchat_monthly_activity, conf.level=.95)
exact2x2(edchat_monthly_activity, conf.level=.95)
mcnemar.test(edchat_monthly_activity, correct = FALSE)
```

So, McNemara's test was not the correct approach because we are not interested in this comparison exactly.

Instead, we wanted to know whether the activity measure (i.e., original-tweets-per-month-per-user) was statistically significantly different after we removed spam from our dataset.

For this comparison, we used the paired samples Wilcoxon test (also known as Wilcoxon signed-rank test), a *non-parametric* alternative to the paired t-test, used to compare *paired* data when the data are not normally distributed. (See http://www.sthda.com/english/wiki/paired-samples-wilcoxon-test-in-r for a very basic tutorial on the Wilcoxon test in R.)
...
paste("All #Edchat tweets"); paste("Mean:", edchat_mean_tweets); paste("SD:", edchat_sd_tweets); paste("Median:", edchat_median_tweets)

edchat_mean_monthly_orig_tweets <- 
    edchat_tweeters_unspammed %>%
    filter(orig_tweets_made_edchat > 0) %>%
    pull(monthly_orig_tweets) %>%
    mean() %>% round(2)
edchat_sd_monthly_orig_tweets <- 
    edchat_tweeters_unspammed %>%
    filter(orig_tweets_made_edchat > 0) %>%
    pull(monthly_orig_tweets) %>%
    sd() %>% round(2)
edchat_median_monthly_orig_tweets <- 
    edchat_tweeters_unspammed %>%
    filter(orig_tweets_made_edchat > 0) %>%
    pull(monthly_orig_tweets) %>%
    median() %>% round(2)

paste("Monthly #Edchat original tweets"); paste("Mean:", edchat_mean_monthly_orig_tweets); paste("SD:", edchat_sd_monthly_orig_tweets); paste("Median:", edchat_median_monthly_orig_tweets)

edchat_mean_monthly_orig_tweets_unspammed <- 
    edchat_tweeters_unspammed %>%
    filter(!is_spam, orig_tweets_made_edchat > 0) %>%
    pull(monthly_orig_tweets) %>%
    mean() %>% round(2)
edchat_mean_monthly_orig_tweets; edchat_mean_monthly_orig_tweets_unspammed

edchat_mean_monthly_retweets <- edchat_tweeters_unspammed$monthly_retweets %>% mean() %>% round(2)
edchat_sd_monthly_retweets <- edchat_tweeters_unspammed$monthly_retweets %>% sd() %>% round(2)
edchat_median_monthly_retweets <- edchat_tweeters_unspammed$monthly_retweets %>% median()

paste("Monthly #Edchat retweets"); paste("Mean:", edchat_mean_monthly_retweets); paste("SD:", edchat_sd_monthly_retweets); paste("Median:", edchat_median_monthly_retweets)
...
wilcoxon_result <- wilcox.test(monthly_orig_tweets ~ is_spam,
                   data = edchat_tweeters_unspammed, 
...
library("ggpubr")
ggboxplot(edchat_tweeters_unspammed, x = "is_spam", y = "monthly_orig_tweets", 
          color = "group", palette = c("#00AFBB", "#E7B800"),
          order = c("before", "after"),
          ylab = "Weight", xlab = "Groups")
```

However, Wilcoxon turns out to not be appropriate because our data are not actually paired; i.e., there are a different number of observations in the 'relevant' and 'spam' goups. This means that what we are actually looking for is how group membership (i.e., relevant tweets vs. spam) predicts the monthly activity score, original-tweets-per-month-per-user.

Look for covariance???

```{r, include=TRUE, echo=FALSE}
model <- glm(monthly_orig_tweets ~ .,
