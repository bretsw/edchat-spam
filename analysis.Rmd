...
n_tweets_unspammed<- edchat_unspammed %>% pull(status_id) %>%
    unique() %>% 
    length()
n_tweeters_unspammed <- edchat_unspammed %>% pull(user_id) %>%
    unique() %>% 
    length()
...
For comparison to other Twitter education hashtags, look at tweets per month per user.

```{r, include=TRUE, echo=FALSE}
time_start_rtweet <- edchat_rtweet_public$created_at %>% min() %>% ymd_hms()
time_end_rtweet <- edchat_rtweet_public$created_at %>% max() %>% ymd_hms()
n_months_rtweet <- (time_end_rtweet - time_start_rtweet) %>% time_length(unit="months")

time_start_unspammed <- edchat_unspammed$created_at %>% min() %>% ymd_hms()
time_end_unspammedt <- edchat_unspammed$created_at %>% max() %>% ymd_hms()
n_months_unspammed <- (time_end_unspammedt - time_start_unspammed) %>% time_length(unit="months")

paste("Tweets per month per user:", 
      round(n_tweets_rtweet_public / n_months_rtweet / n_tweeters_rtweet_public, 2)
      )

paste("Tweets per month per user after removing spammers:", 
      round(n_tweets_unspammed / n_months_unspammed / n_tweeters_unspammed, 2)
      )
```
...
```{r, include=TRUE}
edchat_ focus <- edchat_tweeters %>% filter( 
                                                 like_mean < 1, 
                                                 retweet_mean < 1,
                                                 reply_mean < 1,
                                                 following_ratio > 2
                                                 )
nrow(edchat_closer_look)
View(edchat_closer_look)


edchat_tweeters %>% filter(url_mean > 1.5) %>% nrow()

edchat_tweeters %>% filter(hashtag_mean > 4,
                           like_mean < 1,
                           reply_mean < 1
                           ) %>% nrow()
